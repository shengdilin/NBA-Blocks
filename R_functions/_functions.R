...
player_dist <- function(lastnameA,lastnameB, eventID, rebound.movements) {
  #Functions finds the distance of the player, assumes you have a dataframe rebound.movements with player info
  df <- rebound.movements[which((rebound.movements$lastname == lastnameA | rebound.movements$lastname == lastnameB) & rebound.movements$event.id == eventID),]
...
get_game_clock <- function(lastnameA,eventID,rebound.movements){
  #Function gets the game clock, assumes there is a dataframe rebound.movements with player info
  alldf <- rebound.movements[which((rebound.movements$lastname == lastnameA) & rebound.movements$event.id == eventID),]
...
get_pbp <- function(gameid){
  ...
  URL1 <- paste("https://www.nba.com/game/", gameid, "/play-by-play",sep = "")
...
get_velocity <- function(xloc, yloc, clock){
  diffx <- as.vector((diff(xloc)))
  diffy <- as.vector((diff(yloc)))
  diffClock <- as.vector((diff(clock)))
  diffx2 <- diffx ^ 2
  diffy2 <- diffy ^ 2
  a<- diffx2 + diffy2
  b<-sqrt(a)*abs((1/diffClock)) # as we are taking difference with around .2 second interval
  #(distance in feet per second)
  b
}
